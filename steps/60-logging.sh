...
if ask_yes_no "Log to a FILE instead of the journal?" "y/N"; then
  ...
  supports=false; [[ "$SYSTEMD_VER" =~ ^[0-9]+$ ]] && (( SYSTEMD_VER >= 240 )) && supports=true

  mode="$(ask_choice "  File mode" "append (keep existing content)" "truncate (overwrite on start)")"
  if [[ "$mode" == append* ]]; then
    if $supports; then
      STDOUT="append:${LOGFILE}"; STDERR="append:${LOGFILE}"
    else
      warn "systemd $SYSTEMD_VER lacks append:/truncate: → using file: (truncate on start)."
      STDOUT="file:${LOGFILE}"; STDERR="file:${LOGFILE}"
    fi
  else
    if $supports; then
      STDOUT="truncate:${LOGFILE}"; STDERR="truncate:${LOGFILE}"
    else
      STDOUT="file:${LOGFILE}"; STDERR="file:${LOGFILE}"
    fi
  fi
